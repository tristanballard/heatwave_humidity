...
# Extracts the tmax and shum for region/season combos
# 4-...R in the rcp85 folder does 1970-2005 as a whole, this does 1970-1987, 1988-2014
#request 100GB, 3ish hours
...
model=as.character(threshold.filenames[args,1])
...
#yr.start=1970 #try not to have this be on a leap year
#yr.end=2005
yr.start=1
yr.end=18
...
m=1969
yr.start.real=yr.start+m
yr.end.real=yr.end+m
yr.start.real2=yr.start2+m
yr.end.real2=yr.end2+m
...
for(i in 1:4){ 
  ...
  daily.tmax.2=array(tmax.hw[,,season,yr.start2:yr.end2],dim=c(n.lon,n.lat,n.years*length(season)))
  daily.shum.2=array(shum.hw[,,season,yr.start2:yr.end2],dim=c(n.lon,n.lat,n.years*length(season)))
...
################################################
## Extract the shum/tmax values for the month of interest. 
  #already done above in the first for loop
